...
    document.getElementById('route-creator').style.display = 'flex'; // Usando 'flex' para exibir o container de forma responsiva
...
// Finalizar criação de rota (CORRIGIDO O PAYLOAD: 'ids_estacoes' para 'estacoes')
async function finalizarCriacaoRota() {
    // Estes campos não devem ser null devido à correção no HTML
    const nomeRota = document.getElementById('route-name').value || `Rota ${rotas.length + 1}`;
    const descricaoRota = document.getElementById('route-description').value; // Linha 340 (ou próxima)

    if (rotaAtual.length < 2) {
        // Usando console.error em vez de alert() para seguir boas práticas em aplicações de UI
...
    // CORREÇÃO CRÍTICA: Garante que os IDs das estações são convertidos para o tipo numérico esperado pelo servidor.
    const idsEstacoes = rotaAtual.map(estacao => Number(estacao.id));

    const dados = {
        nome: nomeRota,
        descricao: descricaoRota, // Inclui a descrição
        estacoes: idsEstacoes // <<-- Agora com certeza será um array de números!
    };
...
            // CORRIGIDO: Usando console.error e exibindo a mensagem de erro do servidor
            console.error('Erro ao salvar rota:', erro.mensagem);
            atualizarStatus('ERRO: ' + (erro.mensagem || `Erro na resposta do servidor: ${resposta.status}`));
...
        atualizarStatus('ERRO: Erro ao salvar estação: ' + error.message);
    }
}
...
    }
    
    // CORREÇÃO: Substituindo window.confirm() por mensagem de console/status. 
    // Em aplicações reais, um modal de confirmação customizado seria usado.
    console.warn('Ação de exclusão iniciada para a estação ID:', idEstacao);
    
    // **Atenção:** Se você realmente precisa de uma confirmação do usuário, implemente um modal 
    // customizado. Por enquanto, a exclusão será direta após o clique.
    // Para fins de teste, vou prosseguir. Se precisar de confirmação, me avise para criar o modal.
